...
4. Create a function to *string* **`gameTableTitle`**(*int* **`codeElement`**, *int* **`elementType`**) to generate the table title above. **`elementType`** to determine how many empty space inside the cell. 

5. Store the return *string* as **`gameTable`** and display **`gameTable`**.

6. Generate the a string vector **`secretCode`** using function **`generateCode()`**.
...
2. *string* **`gameTableTitle`**(*int* **`codeElement`**, *int* **`codeRow`**, *int* **`elementType`**)

3. *vector<string>* **`generateCode`**(*int* **`codeElement`**, *int* **`codeRow`**, *int* **`elementType`**)
...
For example, player enter the code "1111", the secret code is "4321". That is one number is correct match. Hence, return a "`#`" on the back.
...
Welcome, `playName`
 ...
 Congratulations, `playName` you have opened the Wood Gate!
...
Welcome, `playName`
 ...
 Thank you for playing this game. Goodbye, `playName`!
...
1. Player can continue the saved uncompleted game, that is when they press [`C`] in the Main page. Then, player will the saved game.
...
2. Player can press [`H`] to ask for help, that is to display the game rule.
...
3. Press [`A`] to view the achievements of all players, the inditifier is `playerName`. It record the total points of each user and the highest points each user have got in each gate.
